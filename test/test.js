...
import { Channel, SlidingBuffer, DroppingBuffer, alts, timeout } from "../src/index.js";
import { _Promise as Promise } from "../src/promises.js";
...
(() => {
  ...
  channel.put(wait(500).then(() => 5));
  channel.put(wait(400).then(() => 4));
  channel.put(wait(200).then(() => 2));
  channel.put(wait(300).then(() => 3));
  channel.put(wait(100).then(() => 1));

  channel.take().then(function(v) { console.log(v); });
  channel.take().then(function(v) { console.log(v); });
  channel.take().then(function(v) { console.log(v); });
  channel.take().then(function(v) { console.log(v); });
  channel.take().then(function(v) { console.log(v); });
